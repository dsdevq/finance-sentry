...
/// primaries, defined once in <see cref="CanonicalCategories"/>). This type only adapts that
...
public static class CategoryTaxonomy
{
    public static readonly IReadOnlyDictionary<string, string> CategoryLabels =
        CanonicalCategories.Definitions.ToDictionary(c => c.Key, c => c.Label, StringComparer.Ordinal);

    public static readonly IReadOnlySet<string> ValidKeys =
        CanonicalCategories.Definitions.Select(c => c.Key).ToHashSet(StringComparer.Ordinal);
