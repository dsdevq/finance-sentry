...
Add a new backend-only module, `FinanceSentry.Modules.Risk`, that stores a versioned per-user `RiskRuleSet` (max position weight, max sleeve weight, min cash buffer, max loss per thesis, max new-position size, turnover budget, allocation targets) and runs a daily Hangfire check of the live book (read via the existing `IBrokerageHoldingsReader` / `ICryptoHoldingsReader` / `IBankingTotalsReader` domain interfaces in `FinanceSentry.Core`) against that rule set. Violations are deterministic pure-function facts — no LLM, no composite score — written to the shared `radar_signals` log via 018's `IRadarSignalWriter` (Core, implemented by Radar) AND to a new `AlertType.PolicyViolation` via the existing `IAlertGeneratorService` contract, with an acknowledged-violation flow so the pre-existing ~46% DRAM concentration becomes a tracked remediation, not daily noise. Three MCP tools (`check_risk_rules`, `get_risk_rules`, `save_risk_rules`) expose the rule set and a promotion-time `Allowed | Refused` verdict that 019's `promote_candidate` (and Ledger) will call before any position is proposed — this feature is MCP-only, no REST surface. The module keeps its own `HoldingSnapshot` history (point-in-time quantity captures on each check run) because no existing module persists holdings history — turnover-budget counting and the add-to-broken-thesis flag (via a new Core `IBrokenThesisReader`, implemented in Research over `IThesisRepository`, mirroring `IWatchlistReader`) are computed from that history. This is the practitioner layer the 2026-07-07 review found missing, and 019's promote flow depends on it as a hard gate.
...
**Testing**: xUnit — unit tests for the pure evaluation functions (`RiskEvaluationService`, `TurnoverTracker`, `AddToBrokenThesisDetector`, `BookSnapshotReader`) covering the seeded 46%-DRAM/25%-cap scenario, the acknowledged-violation path, and the stale-book path; contract tests for the 3 new MCP tools in `FinanceSentry.Mcp.Tests` (agreed tool surface goes 39 → 42). **This feature is MCP-only** — no REST `RiskController`, matching the 017/018/020 precedent; no REST contract tests.
...
|---|---|---|
| I — Modular Monolith + domain interfaces | ✅ PASS | New `FinanceSentry.Modules.Risk` is self-contained (own `RiskDbContext`, own migrations, no ProjectReference to Research). Reads other modules' data exclusively through `FinanceSentry.Core.Interfaces` readers (`IBrokerageHoldingsReader`, `ICryptoHoldingsReader`, `IBankingTotalsReader`) and the new `IBrokenThesisReader` (Core interface, implemented in Research over `IThesisRepository` — mirrors the established `IWatchlistReader` precedent, keeping Risk decoupled from Research internals). Writes go through `IAlertGeneratorService` (Core interface, implemented in Alerts) and `IRadarSignalWriter` (Core interface, implemented in Radar) — no direct reference to `FinanceSentry.Modules.Alerts` or `FinanceSentry.Modules.Radar` internals. |
...
| Testing Discipline | ✅ PASS | Test-First: xUnit unit tests for evaluation logic written before handlers; MCP contract test updated for the 3 new tools (no REST endpoints — MCP-only feature, per 017/018/020 precedent) |
| Versioning | ✅ PASS | MCP tool surface changed (39 → 42); no backend REST version bump (no new REST endpoints); no frontend version bump (no frontend changes) |
...
│       └── IBrokenThesisReader.cs                             [NEW] Core-facing read of broken theses (017), implemented in Research (mirrors IWatchlistReader) — keeps Risk decoupled from Research internals
│
...
│   └── Application/Services/BrokenThesisReader.cs             [NEW] IBrokenThesisReader impl over IThesisRepository
│
├── FinanceSentry.Modules.Risk/                                [NEW MODULE — MCP-only, no REST controller]
│   ├── RiskModule.cs                                          module DI registration (mirrors WealthModule.cs / RadarModule.cs)
│   ├── API/
│   │   └── Responses/
│   │       └── CheckRiskRulesResponseDto.cs                   MCP-facing shape (compliance report OR proposal verdict)
│   ├── Application/
│   │   ├── Commands/
...
│
├── FinanceSentry.API/
│   ├── FinanceSentry.API.csproj                                  [MODIFIED] ProjectReference to Modules.Risk
│   └── Migrations/MigrationExtensions.cs                         [MODIFIED] MigrateContext<RiskDbContext>
└── FinanceSentry.Mcp/
    ├── FinanceSentry.Mcp.csproj                                  [MODIFIED] ProjectReference to Modules.Risk
    ├── Program.cs                                                [MODIFIED] add Modules.Risk assembly to moduleAssemblies
    └── Tools/
        ├── CheckRiskRulesTool.cs                                 [NEW] check_risk_rules
        ├── GetRiskRulesTool.cs                                   [NEW] get_risk_rules
        └── SaveRiskRulesTool.cs                                  [NEW] save_risk_rules
...
backend/tests/
├── FinanceSentry.Modules.Risk.Tests/
...
│   ├── BookSnapshotReaderTests.cs                                 per-source degrade → staleness flag
│   └── AddToBrokenThesisTests.cs                                 quantity-increase-after-break vs before-break ordering
└── FinanceSentry.Mcp.Tests/
    └── ContractTests/ToolNameContractTests.cs                     [MODIFIED] AgreedToolSurface 39 → 42 (+check_risk_rules, get_risk_rules, save_risk_rules)
```

Module registration (DI + Hangfire recurring job) and MCP tool discovery are both reflection-based (`ModuleRegistrationExtensions.AddAllModules`, `JobRegistrationExtensions`, MCP's `WithToolsFromAssembly`) — once the ProjectReferences and the `moduleAssemblies` array entry are in place, `RiskModule.ModuleRegistrar`/`JobRegistrar` and the three `[McpServerToolType]` tool classes are picked up automatically; no other central registration edits are required.

**Dependency on 017/018/019**: 022 does not block on 017 (thesis-broken state) shipping first — FR-006 (add-to-broken-thesis) degrades to "no theses to check" if `IBrokenThesisReader` has no broken records yet. **018 is in this feature's base branch**, so signals go to the shared `radar_signals` log via `IRadarSignalWriter` (Core, implemented by Radar) from day one — not the "Alerts-only until 018 ships" fallback in the original spec note. 022 is a hard **gate** for 019 (`promote_candidate` must call `check_risk_rules` before any promotion) — 019 cannot ship its promote flow before 022's `CheckRiskRulesQuery` proposal path exists.
