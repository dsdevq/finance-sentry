...
- [ ] T067 [P] Run a full QA sweep using Playwright MCP per `quickstart.md` walkthrough: 4 golden paths + 10 edge cases. Record any deviations as bugs and fix before sign-off. *(Sweep executed 2026-08-06 via scripted Playwright — tick blocked on fixes per "fix before sign-off". Drift: Plaid hidden from picker (TrueLayer added), IBKR now OAuth 1.0a — 3 quickstart cases obsolete; real-provider success paths environment-limited locally. Disconnect golden path, connected badges, and SC-005 security spot-check pass. 4 open bugs: (1) Monobank invalid token → 500 (`MonobankHttpClient` maps only 401, real API returns 403); (2) stale/wrong-provider error banner (`ConnectStore` leaves `status='error'` with null code, crypto/broker never set `selectedProvider`, `INVALID_CREDENTIALS` vs `BINANCE_INVALID_CREDENTIALS` registry mismatch); (3) form state destroyed on failed submit (`@if isBusy` swaps form for syncing-state) + Monobank `tokenError` computed() over non-signal form state never fires; (4) `cmn-dialog` min-w 512px clips at 360px viewport.)*
