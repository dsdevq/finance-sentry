    ...
    IQueryHandler<GetAnalystActionsQuery, AnalystActionsResult> handler)
{
    ...
    [Description("Query street/analyst actions (upgrades, downgrades, coverage initiations, price-target changes, reiterations) ingested nightly from the MarketBeat market-wide sweep. Market-wide — ticker is OPTIONAL; omit it to see actions across the whole covered universe, including names Denys does not hold. Every row carries its source for the 'every claim carries a fresh source' rule. The 'coverage' field distinguishes 'notInUniverse' (ticker isn't tracked) from an empty result (tracked, but no recent actions). When a ticker IS given, the response also carries 'recommendationTrends' — the latest monthly analyst-consensus counts (strongBuy/buy/hold/sell/strongSell, source: Finnhub structured API) for citing consensus direction alongside per-action events.")]
